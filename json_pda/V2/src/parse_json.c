    ...
    )
{
  ...
  *ptr_X = X; *ptr_nX = nX;
  if ( *X == 'n' ) { 
    status = parse_null(ptr_X, ptr_nX, ptr_J);
  }
  else if ( ( *X == 't' ) || ( *X == 'f' ) ) { 
    status = parse_boolean(ptr_X, ptr_nX, ptr_J); 
  }
  else if ( ( isdigit(*X) ) || ( *X == '-' ) ) {
    status = parse_number(ptr_X, ptr_nX, ptr_J); 
  }
  else if ( *X == '"' ) { 
    status = parse_string(ptr_X, ptr_nX, ptr_J); 
  }
#ifdef XXX
  else if ( *X == '[' ) { 
